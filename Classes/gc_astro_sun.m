...
//////////////////////////////////////////////////////////////////////////
//
// gc_time  vct [in] - valid must be each member of this structure
//
...
void SunPosition(gc_time vct, gc_earth ed, gc_sun * sun, DAYHOURS DayHours)
{
	...
	//	eventdef = 0.30902;
	// center of the sun on the horizont
	//	eventdef = 0.01454;
...
void SunCalc(gc_time vct, gc_earth earth, gc_sun * sun)
{
	...
	sun->arunodaya = gc_daytime_initFromDegTime(sun->arunodaya_deg + earth.tzone*15.0);
	// sunrise
	sun->rise = gc_daytime_initFromDegTime(sun->sunrise_deg + earth.tzone*15.0);
	// noon
	sun->noon = gc_daytime_initFromDegTime((sun->sunset_deg + sun->sunrise_deg)/2  + earth.tzone*15.0);
	// sunset
	sun->set = gc_daytime_initFromDegTime(sun->sunset_deg + earth.tzone*15.0);
	// length
	sun->length = gc_daytime_initFromDegTime(sun->length_deg);
...
int gAyaType = 1; // default is 1
int gSanType = 2; // default is 2
...
NSString * GetSankMethodName(int i)
{
	NSString * snam[] = {
